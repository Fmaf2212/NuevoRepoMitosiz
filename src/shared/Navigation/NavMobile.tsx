...
const NavMobile: React.FC<NavMobileProps> = ({
      ...
      <ul className="nav-mobile-sub-menu pl-6 pb-1 text-base">
        {item.children?.map((i, index) => (
          <Disclosure key={i.href + index} as="li">
            ...
            >
              <span
                className={`py-2.5 text-white ${!i.children ? "block w-full" : ""}`}
  ...
  const _renderItem = (item: NavItemType, index: number) => {
    return (
      ...
      >
        <NavLink
          className={({ isActive }) =>
            `flex w-full items-center py-2.5 px-4 font-medium uppercase tracking-wide text-sm hover:bg-[#FFCE00] dark:hover:bg-slate-800 rounded-lg ${
              isActive ? "text-secondary" : ""
            }`
          }
          to={{
            pathname:  item.name!=="Compras" ? item.href || undefined : undefined,
          }}
        >
          <span
  ...
  return (
    <div className="overflow-y-auto w-full h-screen py-2 transition transform shadow-lg ring-1 dark:ring-neutral-700 dark:bg-neutral-900 divide-y-2 divide-[#FFCE00] dark:divide-neutral-800" style={{background: "linear-gradient(to top, #3A0C7F, #6800D1, #653EFF)"}}>
      <div className="py-6 px-5 flex justify-center">
        <Link
          to="/"
          className={`ttnc-logo inline-block text-slate-600`}
        >
          {logoImg && (
            <img
